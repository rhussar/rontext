  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="px-5 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
        {remaining.length === 0
          ? "Nothing to review"
          : `${remaining.length} ${remaining.length === 1 ? "person" : "people"} found in your messages, mail and calendar`}
      </div>
        ...
        {remaining.length === 0 ? (
          <div className="flex flex-col items-center gap-2 pt-16 text-center">
            ...
            <p className="max-w-sm text-[13.5px] text-muted-foreground">
              People you exchange messages or email with, or meet with, who aren’t in your
...
function evidence(item: CandidateItem): string {
  const parts =
    item.source === "calendar"
      ? [
          `Calendar · ${item.messageCount.toLocaleString()} ${
            item.messageCount === 1 ? "meeting" : "meetings"
          }`,
        ]
      : [
